...
function VistaAdminDelegados() {
  ...
  const cargar = () => {
    ...
    fetch(`${API}/admin/delegados/detalle`).then(r=>r.json())
...
export default function Dashboard() {
  ...
  const cargarDefensas = () => {
    ...
    setLoading(true);
    const url =
      usuario.rol === 0 ? `${API}/admin/defensas` : `${API}/defensas/${usuario.id}`;
    fetch(url)
      .then((r) => r.json())
      .then((d) => {
  ...
  const handleEvidencia = async ({ defensa, comentarios }: { defensa: Defensa; comentarios?: string }) => {
  ...
  if (usuario.rol === 0) {
    ...
    return (
      <div className="admin-shell">
        <AdminSidebar usuario={usuario} active={nav} onChange={handleNav} onLogout={logout} />
  ...
  const renderDelegado = () => {
    ...
    switch (nav) {
      case "inicio":
        return (
          <VistaInicio usuario={usuario} defensas={defensas} loading={loading} onNav={handleNav} />
        );
      case "nuevas":
        return (
          <VistaNuevas
      ...
      case "completadas":
        return (
          <VistaCompletadas usuario={usuario} defensas={defensas} loading={loading} />
        );
      case "perfil":
